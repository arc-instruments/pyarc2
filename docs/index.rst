...
``pyarc2`` should be available from pip. Just run
...
A firmware is required to open a connection to the instrument. This is not
typically provided by ``pyarc2`` but it is available from our website_.
Compiled distributions of higher-level applications, such as arc2control_
would typically also include the latest firmware.
